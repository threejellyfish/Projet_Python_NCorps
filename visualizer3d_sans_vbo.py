...
class Visualizer3D:
    ...
    def run_with_updater(self, updater_func, dt=0.01):

        self.running = True
        
        print("Contrôles :")
        print("  - Clic gauche + souris : rotation")
        print("  - Molette : zoom")
        print("  - ESC : quitter")
        ...
        while self.running:
            ...
            if updater_func is not None:
                new_points, new_colors, new_luminosities = updater_func(dt)
                self.update_points(new_points, new_colors, new_luminosities)
